...
public class FungalFootingLayer<S extends HumanoidRenderState, M extends HumanoidModel<S>> extends RenderLayer<S, M> {
	...
	public FungalFootingLayer(RenderLayerParent<S, M> renderer) {
		super(renderer);
	...
	public void submit(PoseStack poseStack, SubmitNodeCollector nodeCollector, int packedLight,
		...
		if (renderState.getRenderDataOrDefault(ClientHandler.HAS_FUNGAL_INFECTION, false)) {
			if (!renderState.isInvisible ) {
				...
				final ModelPart leftLeg = parentModel.leftLeg;
				final ModelPart rightLeg = parentModel.rightLeg;
